...
const FEATURES: FeatureOption[] = [
  ...
  {
    id: 'framer',
    label: 'Apple-Grade Framer Motion Fluidity',
  ...
  {
    id: 'billing',
    label: 'Stripe Payments & Subscription Rails',
    category: 'scale',
    description: 'Customer portal, webhook lifecycle listener, tiered subscriptions, and automated license provisioning.',
    extraWeeks: 1.0,
    priceDelta: [800, 1400],
    complexityDelta: 1,
    recommendedStack: 'Stripe SDK + Webhooks',
  },
  {
    id: 'realtime',
  ...
  {
    id: 'seo',
    label: 'Sub-Second Edge SSR & Lighthouse 100',
    category: 'core',
...
const VELOCITIES = [
  ...
  },
  {
    ...
    factor: 1.0,
    tag: '🎯 Recommended',
    description: 'Weekly demo milestones, staged user testing, and collaborative Slack/Discord channel.',
  },
  {
    id: 'deep',
...
export function BlueprintEstimator() {
  ...
  const calculations = useMemo(() => {
    ...
    const calculatedWeeks = Math.max(2, Math.round(totalWeeks * velocity.factor));
    const normalizedComplexity = Math.min(5, Math.round(complexityScore * 10) / 10);
    ...
    if (selectedFeatures.includes('billing')) stackItems.add('Stripe API / Webhooks');
      ...
      const res = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({
          ...
          projectContext: {
            ...
            scope: `${calculations.weeks} weeks timeline, complexity score ${calculations.complexity}/5`,
          },
        }),
      });
...
- **Estimated Timeline**: ${calculations.weeks} Weeks (${calculations.velocity.name})
- **Complexity Rating**: ${calculations.complexity} / 5.0
- **Investment Bracket**: $${calculations.minPrice.toLocaleString()} – $${calculations.maxPrice.toLocaleString()} USD
...
`;
  ...
  const handleTransferToInquiry = () => {
    ...
    const queryParams = new URLSearchParams({
      ...
      budget: `$${calculations.minPrice.toLocaleString()} - $${calculations.maxPrice.toLocaleString()}`,
      weeks: `${calculations.weeks} weeks`,
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 pt-3 hairline-t flex items-center justify-between text-[11px] text-[var(--ink-soft)]">
                      <span>Base: ~{arch.baseWeeks} wks</span>
                      <span className="font-semibold text-[var(--ink)]">
                        ${arch.basePrice[0].toLocaleString()}+
                      </span>
                    </div>
                  </button>
                );
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="pt-1.5 flex items-center gap-2 text-[10px] text-[var(--green)] font-mono">
                        <span>+{feat.extraWeeks} wks</span>
                        <span>•</span>
                        <span>+${feat.priceDelta[0]}–${feat.priceDelta[1]}</span>
                      </div>
                    </div>
                  </button>
                );
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-2xl bg-[var(--surface)]/80 hairline-all space-y-1">
                ...
                <div className="text-2xl sm:text-3xl font-display font-bold text-[var(--ink)]">
                  ~{calculations.weeks} <span className="text-sm font-sans font-normal text-[var(--ink-soft)]">wks</span>
                </div>
              ...
              <div className="p-4 rounded-2xl bg-[var(--surface)]/80 hairline-all space-y-1">
                ...
                <div className="text-lg sm:text-2xl font-display font-bold text-[var(--green)]">
                  ${calculations.minPrice.toLocaleString()}–${calculations.maxPrice.toLocaleString()}
                </div>
                <p className="text-[10px] text-[var(--ink-soft)]">
